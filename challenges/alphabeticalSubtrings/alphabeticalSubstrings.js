...
const recursiveSubstringCheck = (array, currentIndex, currentSubstringArray = []) => {
  ...
  if (alphabet[nextLetter] - alphabet[currentLetter] !== 1) { return [...currentSubstringArray, array[currentIndex]] }
...
const longestAlphabeticalSubstring = (string) => {
  ...
  for (let i = 0; i < filteredArr.length; i += 1) {
    const alphabeticalSubstringArray = recursiveSubstringCheck(filteredArr, i)
    if (alphabeticalSubstringArray.length > currentLongestSubstring.length) { currentLongestSubstring = alphabeticalSubstringArray }
  }

  return currentLongestSubstring.join('')
}

export default longestAlphabeticalSubstring
